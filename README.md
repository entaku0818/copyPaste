...
- 👑 **無制限履歴**
  - 件数の上限なし（無料版は直近3日・20件まで）

- 👑 **高度な検索機能**
  - テキスト、URL、ファイル名から瞬時に検索
  - 大文字小文字を区別しない
...
- 👑 **キーボードエクステンション**
  - どのアプリでもキーボードから履歴にアクセス
  - 素早いペースト操作

- **テキスト変換**
  - 大文字/小文字・改行除去・空白トリム（無料）
  - 👑 スネークケース/キャメルケース・全角/半角・Markdown除去・URLエンコード/デコード（Pro）
  - 👑 キーボードから変換して貼付け（Pro）
