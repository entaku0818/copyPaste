...
export const submitFeedback = onCall(
  ...
  async (request) => {
    ...
    const payload = {
      username: "ClipKit Feedback",
      icon_emoji: ":clipboard:",
      text: `${emoji} *ClipKit に新しいフィードバックが届きました*`,
      attachments: [
        {
          color: data.category === "バグ報告" ? "#e74c3c" : "#2ecc71",
          fields: [
            {title: "カテゴリ", value: data.category, short: true},
            {
              title: "バージョン",
            ...
            ...(data.email ? [{title: "メールアドレス", value: data.email, short: true}] : []),
            {title: "内容", value: data.message, short: false},
          ],
        },
      ],
    };
